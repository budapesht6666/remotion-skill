...
export const AppScene: React.FC<{ shot: AdShot }> = ({ shot }) => {
  ...
  // субтитров, иначе текст ложится на экран аппки и нечитаемы оба. При 540 px
  // рамка занимает 1193 px по высоте и вместе со сдвигом вверх (paddingBottom)
  // укладывается в 194…1386 — субтитры начинаются с 1500.
  const width = 540;
...
const FreeBadge: React.FC<{ text: string }> = ({ text }) => {
  ...
  // Плашка живёт в зазоре между низом телефона (1386) и строкой субтитров
  // (1500). Поверх экрана её ставить нельзя: она закрыла бы то самое, ради чего
  // экран в кадре, — настоящие строки сигналов.
  return (
    <AbsoluteFill style={{ justifyContent: "flex-end", alignItems: "center", paddingBottom: 464 }}>
      <span
        style={{
          fontFamily: BRAND_FONT,
          fontWeight: WEIGHT.medium,
